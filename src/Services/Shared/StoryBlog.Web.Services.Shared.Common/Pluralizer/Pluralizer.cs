...
using StoryBlog.Web.Services.Shared.Common.Pluralizer.Rules;

namespace StoryBlog.Web.Services.Shared.Common.Pluralizer
{
    public class Pluralizer
    {
        private readonly Dictionary<Regex, string> pluralRules = PluralRules.GetRules();
        private readonly Dictionary<Regex, string> singularRules = SingularRules.GetRules();
        private readonly List<string> uncountables = Uncountables.GetUncountables();
        private readonly Dictionary<string, string> irregularPlurals = IrregularRules.GetIrregularPlurals();
        private readonly Dictionary<string, string> irregularSingles = IrregularRules.GetIrregularSingulars();
        private readonly Regex replacementRegex = new Regex("\\$(\\d{1,2})");

        public string Pluralize(string word)
        {
            return Transform(word, irregularSingles, irregularPlurals, pluralRules);
        }

        public string Singularize(string word)
        {
            return Transform(word, irregularPlurals, irregularSingles, singularRules);
        }
        ...
        internal string ApplyRules(string token, string originalWord, Dictionary<Regex, string> rules)
        {
            ...
            if (string.IsNullOrEmpty(token) || uncountables.Contains(token))
