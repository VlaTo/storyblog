...
namespace StoryBlog.Web.Blazor.Reactive.Concurrency
{
    ...
    {
        private static readonly Lazy<NewThreadScheduler> Instance;
        ...
        static NewThreadScheduler()
        {
            Instance = new Lazy<NewThreadScheduler>(() => new NewThreadScheduler());
        }
        ...
        private sealed class Periodic<TState> : IDisposable
        {
            ...
            private readonly Func<TState, TState> action;

            private readonly object cancel = new object();
            ...
            public void Run()
            {
                while (false == done)
                {
                    var timeout = Scheduler.Normalize(next - stopwatch.Elapsed);

                    lock (cancel)
                    {
                        if (Monitor.Wait(cancel, timeout))
                        {
                            return;
                        }
                    }
