...
namespace StoryBlog.Web.Blazor.Reactive
{
    ...
    public sealed class AnonymousObserver<T> : ObserverBase<T>
    {
        ...
        protected override void OnErrorCore(Exception error) => onError(error);

        /// <summary>
        /// Calls the action implementing <see cref="IObserver{T}.OnCompleted()"/>.
        /// </summary>
        protected override void OnCompletedCore() => onCompleted();

        internal ISafeObserver<T> MakeSafe() => new AnonymousSafeObserver<T>(onNext, onError, onCompleted);
    }
}