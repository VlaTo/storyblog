...
namespace StoryBlog.Web.Blazor.Reactive
{
    ...
    {
        private static readonly Lazy<ISystemClock> ServiceSystemClock;
        private static readonly Lazy<INotifySystemClockChanged> ServiceSystemClockChanged;
        ...
        static SystemClock()
        {
            ServiceSystemClock = new Lazy<ISystemClock>(InitializeSystemClock);
            ServiceSystemClockChanged = new Lazy<INotifySystemClockChanged>(InitializeSystemClockChanged);
            SystemClockChanged = new HashSet<WeakReference<LocalScheduler>>();
        }
        ...
        private static ISystemClock InitializeSystemClock() => new DefaultSystemClock();
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal static void Register(LocalScheduler scheduler)
        {
            ...
            lock (SystemClockChanged)
            {
                ...
                {
                    _systemClockChangedHandlerCollector = ConcurrencyAbstraction.Current.StartPeriodicTimer(CollectHandlers, TimeSpan.FromSeconds(30));
                }
        ...
        private static void CollectHandlers()
        {
            ...
            lock (SystemClockChanged)
            {
                ...
                {
                    if (false == handler.TryGetTarget(out _))
                    {
                        if (null == remove)
                        {
